...
class Bill_App:
    def __init__(self, root):
        ...
        self.Blood_Pressure = ["Lisinopril","Amlodipine","Metoprolol","Hydrochlorothiazide"]
        ...
        self.SubVitamin =["Multivitamins","Vitamin D2","Omega-3 Fish Oil","Probiotics"]
        self.price_Multivitamins=200
        self.price_VitaminD2=100
        self.price_OmegaFishOil=250
        self.price_Probiotics=300

        self.FirAid = ["Band-Aids","Antiseptic Wipes","Gauze Pads","Adhesive Tape"]
        ...
        self.SubThermometers =["Digital Oral Thermometer","Infrared Forehead Thermometer","Ear Thermometer","Disposable Thermometer Covers"]
        self.price_DigitalOral=2000
        self.price_InfraredForhead=1000
        self.price_Ear=2500
        self.price_Disposable=500

        self.BloodMonitor = ["Accu-Chek Guide","OneTouch Verio","Freestyle Libre","Contour Next"]
        self.price_Accu=100
        self.price_OneTouch=200
        self.price_Free=300
        self.price_Contour=400

        self.Nebulizer = ["Philips Respironics Innospire","DeVilbiss Pulmo-Aide","Medline Aeromist Plus","Omron MicroAir"]
    ...
    def price(self,event=""):
        ...
        if self.Combo_Product.get()=="Hydrochlorothiazide":
        ...
        if self.Combo_Product.get()=="Omega-3 Fish Oil":
        ...
        if self.Combo_Product.get()=="Antiseptic Wipes":
        ...
        if self.Combo_Product.get()=="Digital Oral Thermometer":
            self.Combo_Price.config(value=self.price_DigitalOral)
            self.Combo_Price.current(0)
            self.qty.set(1)

        if self.Combo_Product.get()=="Infrared Forehead Thermometer":
            self.Combo_Price.config(value=self.price_InfraredForhead)
            self.Combo_Price.current(0)
            self.qty.set(1)

        if self.Combo_Product.get()=="Ear Thermometer":
            self.Combo_Price.config(value=self.price_Ear)
            self.Combo_Price.current(0)
            self.qty.set(1)

        if self.Combo_Product.get()=="Disposable Thermometer Covers":
            self.Combo_Price.config(value=self.price_Disposable)
            self.Combo_Price.current(0)
            self.qty.set(1)

        if self.Combo_Product.get()=="Accu-Chek Guide":
        ...
        if self.Combo_Product.get()=="Freestyle Libre":
        ...
        if self.Combo_Product.get()=="Philips Respironics Innospire":
            self.Combo_Price.config(value=self.price_Philip)
            self.Combo_Price.current(0)
            self.qty.set(1)

        if self.Combo_Product.get()=="DeVilbiss Pulmo-Aide":
            self.Combo_Price.config(value=self.price_Devilbee)
            self.Combo_Price.current(0)
            self.qty.set(1)

        if self.Combo_Product.get()=="Medline Aeromist Plus":
